...
public class ManagerFoodUpdateService implements FoodService {

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		// int foodid = Integer.parseInt(request.getParameter("food_id"));
		// String foodkind = request.getParameter("food_kind");
		// String foodname = request.getParameter("food_name");
		// int foodprice = Integer.parseInt(request.getParameter("food_price"));
		// int foodcount = Integer.parseInt(request.getParameter("food_count"));
		// String foodimage = request.getParameter("food_image");
		// dao.updatefood(foodid, foodkind, foodname, foodprice, foodcount, foodimage);
...
//		  출처: https://wowstudy.tistory.com/17 [스터디 용 블로그:티스토리]
		...
		String foodid = imageUp.getParameter("food_id");
		...
		//FDtoFood fdto = dao.getFood(foodid);
		dao.updatefood(Integer.parseInt(foodid), food_kind, food_name, Integer.parseInt(food_price), Integer.parseInt(food_count), filename);
	}
